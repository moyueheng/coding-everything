...
1. **探索项目上下文** — 检查文件、文档、最近提交，并发 subagent 进行大量调研探索；先查现有实现、现有工具、现有模式，禁止一上来假设必须新写
2. **提出澄清问题** — 一次一个问题，理解目的/约束/成功标准
3. **提出 2-3 种方案** — 说明权衡并给出推荐
4. **展示设计** — 按复杂度分节展示，每节后获得用户批准
5. **编写设计文档** — 保存到 `docs/plans/YYYY-MM-DD-<主题>-design.md`
6. **审查设计文档** — 使用 [spec-review-checklist.md](spec-review-checklist.md) 做怀疑式审查；发现问题就回到设计文档修正，直到通过
7. **过渡到实现** — 调用 dev-writing-plans skill 创建实施计划
...
digraph brainstorming {
    ...
    "编写设计文档" -> "审查设计文档";
    "审查设计文档" -> "设计文档通过？";
    "设计文档通过？" -> "编写设计文档" [label="否，修正"];
    "设计文档通过？" -> "调用 dev-writing-plans skill" [label="是"];
}
```
...
**设计审查闭环：**
- 写完设计文档后，必须立即使用 [spec-review-checklist.md](spec-review-checklist.md) 做一次怀疑式审查
- 发现问题就修改设计文档，再重新审查
- 只有设计文档通过审查，才能进入 `dev-writing-plans`
