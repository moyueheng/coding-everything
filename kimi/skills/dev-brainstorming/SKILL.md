...
7. **Spec review loop** — 分派 spec-document-reviewer subagent；修复问题并重新分派，直到通过（最多 3 轮迭代）
...
digraph brainstorming {
    ...
    "编写设计文档" -> "Spec review loop";
    "Spec review loop" -> "Spec review 通过？";
    "Spec review 通过？" -> "Spec review loop" [label="发现问题，\n修复并重新分派"];
    "Spec review 通过？" -> "用户审查 spec？" [label="通过"];
...
1. 分派 spec-document-reviewer subagent（见 spec-document-reviewer-prompt.md）
2. 如果发现问题：修复、重新分派、重复直到通过
3. 如果循环超过 3 轮，提交给人类指导
