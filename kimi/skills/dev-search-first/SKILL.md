...
本 skill 触发后，所有本地与联网检索都必须调用 `tool-mgrep`。
</EXTREMELY-IMPORTANT>
...
digraph search_first {
    "要新增能力？" [shape=diamond];
    "调用 tool-mgrep 做本地/联网检索" [shape=box];
    ...
    "要新增能力？" -> "调用 tool-mgrep 做本地/联网检索" [label="是"];
    "调用 tool-mgrep 做本地/联网检索" -> "存在高质量现成方案？";
