...
1. **创建失败测试用例（必须调用 dev-tdd）**
   - **读取并遵循 `dev-tdd` skill**
   - 编写 RED 测试：展示 bug 的最小测试
   - 验证 RED：运行测试确认它失败于预期原因
   - 最简单的可能重现
   - 可能的话自动化测试
   - 没有框架时用一次性测试脚本
   - 修复前必须有
...
1. **测试证明修复** - 没有测试，你不知道是否真正修复了
2. **测试防止回归** - 同样的 bug 不会再次出现
3. **测试是文档** - 记录了 bug 的行为和修复
4. **强制验证** - RED→GREEN 确保你理解问题
...
**相关 skill：**
- **dev-tdd** - 用于创建失败测试用例（阶段 4，步骤 1）**必须调用**
