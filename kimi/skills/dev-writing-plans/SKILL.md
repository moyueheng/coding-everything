...
1. **单遍整体审查** — 分派 plan-document-reviewer subagent（见 plan-document-reviewer-prompt.md），提供精心设计的审查上下文（绝不要你的会话历史）
2. 如果 ❌ 发现问题：修复问题，重新分派 reviewer 审查整个计划
3. 如果 ✅ 通过：进入执行交接

**审查循环指导：**
- 同一个写计划的 agent 修复问题（保持上下文）
- 如果循环超过 3 轮，提交给人类指导
- reviewer 是顾问性质的 — 如果你认为反馈不正确，解释分歧
