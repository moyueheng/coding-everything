...
### 步骤 0：确认工作区
默认直接在当前分支执行计划，不自动创建 git worktree。

仅当用户明确要求隔离工作区、并行分支验证或临时试验时，才调用 `dev-git-worktrees`。

原因：设计文档或实施计划如果是在当前工作区生成但尚未提交，新建 worktree 后这些未提交文件不可见，容易导致执行阶段丢失上下文。

### 步骤 1：加载和审查计划
1. 读取计划文件
2. 批判性审查 - 识别对计划的任何问题或担忧
3. 如有担忧：在开始前向你的 human partner 提出
4. 如无担忧：创建 TodoWrite or SetTodoList or update_plan 并继续
...
1. **读取任务时**
   - 注意任务中的 `> **给 Agent：** 此任务使用 dev-tdd` 标记
...
**必需工作流 skill：**
- **dev-git-worktrees** - 可选：仅当用户明确要求隔离工作区时使用
