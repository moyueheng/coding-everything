...
description: 用户明确要求隔离工作区、并行分支验证或临时试验时使用 - 创建隔离的 git worktree
---
...
**项目默认：** 不为执行计划自动创建 git worktree。除非用户明确要求隔离工作区，否则直接在当前分支修改。

**已知交接问题：** 如果设计文档或实施计划是在当前工作区生成但尚未提交，新建 worktree 后这些未提交文件不可见，容易导致执行阶段丢失上下文。上游 superpowers 已有相关讨论，后续再评估是否恢复自动 worktree 流程。
...
| 情况 | 行动 |
|-----------|--------|
| 用户明确要求创建 worktree | 使用 `~/.agents/worktrees/<项目名称>/<分支名>` |
| 执行计划但用户未要求隔离 | 直接在当前分支修改，不调用此 skill |
...
**被以下调用：**
- **dev-brainstorming**（阶段 4）- 仅当用户明确要求隔离工作区时调用
- **dev-subagent-driven-development** - 仅当用户明确要求隔离工作区时调用
- **dev-executing-plans** - 默认不调用；仅当用户明确要求隔离工作区时调用
- 任何需要隔离工作区的 skill
